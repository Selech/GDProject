...
public class PlayerControl : MonoBehaviour {
	...
	void Start () {
		suck = GameObject.Find ("TheBlackhole").transform.position;
		weapon = transform.GetComponentInChildren<PushWeapon> ();
	}
	
	// Update is called once per frame
	void Update () {
		transform.position = Vector3.MoveTowards (transform.position, suck, 0.01f); 
	...
	public void asteroidsControl(){
		...
		if (Input.GetKey (left)) {
			...
			if (target.transform.position.y - ship.transform.position.y > -1){
